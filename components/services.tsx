...
const services = [
  ...
  },
  {
    icon: Wrench,
    title: 'Yedek Parça',
    description: 'Birçok farklı markaya ait iş ve istif makineleri için hızlı yedek parça temini sağlıyoruz.',
  },
  {
    icon: Users,
    title: 'Servis ve Teknik Destek',
    description: 'Deneyimli kadromuzla hızlı servis ve teknik destek hizmeti sunuyoruz. Projenizin her aşamasında yanınızdayız.',
  },
];

export function Services() {
  return (
    <SectionWrapper id="hizmetler" className="py-24 lg:py-32 bg-[#F2F1ED]">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
          {services.map((service, index) => (
            <motion.div
              ...
              whileHover={{ y: -5 }}
              className="group bg-white p-8 rounded-lg border border-[#E5E5E5] hover:border-[#1E5AA8] hover:shadow-xl transition-all duration-300"
            >
              <div className="w-14 h-14 rounded-lg bg-[#EEF3FB] flex items-center justify-center mb-6 group-hover:bg-[#1E5AA8] transition-colors">
                <service.icon className="w-7 h-7 text-[#1E5AA8] group-hover:text-white transition-colors" />
              </div>
              <h3 className="font-heading text-xl text-[#2B2B2B] mb-3">
                {service.title}
              </h3>
